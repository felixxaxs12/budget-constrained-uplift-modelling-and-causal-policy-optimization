...
The comparison fixes the data split, model selection procedure, ranking scores, capacities, tie-breaking rule, estimands, and contrasts before the evaluation routine loads test outcomes or joins them to predictions. Three quantities remain separate throughout the analysis: the complete-source difference-in-means average treatment effect, held-out augmented inverse-probability-weighted (AIPW) policy value, and a centered fixed-propensity inverse-probability-weighted (IPW) Qini diagnostic. This separation prevents a ranking metric or an in-sample effect estimate from being presented as policy performance.
...
Empirical welfare maximization frames treatment assignment as direct optimization over a policy class \citep{kitagawa2018treatment}. Related work derives policy learners and regret guarantees from semiparametric scores, including settings with budget or structural constraints \citep{athey2021policy}. Doubly robust policy evaluation combines a response model with importance weighting so that errors in one nuisance component can be offset under suitable conditions \citep{dudik2011dr}. The present experiment is simpler because assignment probability is known and constant. Its policy class is a fixed set of score rankings with exact capacity cutoffs.
...
\resizebox{\textwidth}{!}{\input{tables/test_sample.tex}}
\end{table}
...
\]
Both nuisance predictions exclude row $i$. A LightGBM regressor is then fitted to the out-of-fold pseudo-outcomes. Final arm-specific nuisance models use all development rows with their already selected round counts. The visit models evaluate the same conversion policies; they do not generate another set of rankings.

\subsection{Held-out evaluation}

Models, ranking rules, and capacities were selected with training and validation data. The evaluation routine then loaded test covariates and computed score and nuisance predictions. Only after those predictions existed did it load test outcome columns and align them by row identifier. Test outcomes did not enter model fitting, early stopping, score selection, or capacity selection.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.84\textwidth]{../results/figures/balance.png}
...
\label{tab:ate}
\resizebox{\textwidth}{!}{\input{tables/full_sample_ate.tex}}
\end{table}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{../results/figures/policy_values.png}
...
\resizebox{\textwidth}{!}{\input{tables/conversion_policy_values.tex}}
\end{table}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../results/figures/conversion_policy_contrasts.png}
...
\resizebox{\textwidth}{!}{\input{tables/conversion_contrasts.tex}}
\end{table}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../results/figures/qini_curves.png}
...
\label{tab:qini}
\input{tables/qini_coefficients.tex}
\end{table}
...
\resizebox{\textwidth}{!}{\input{tables/estimator_scope.tex}}
\end{table}
...
\section*{Code, data, and tools}

Code, configuration, and aggregate results are available in the
...
\centering
\captionof{table}{Paired visit policy-value contrasts per 100,000 eligible test users. Policies are ranked by conversion; each contrast is treated-response minus the comparator. Cells report estimate [pointwise 95\% percentile interval].}
\label{tab:visit-contrasts}
\resizebox{\linewidth}{!}{\input{tables/visit_contrasts.tex}}
\end{minipage}
\end{center}
